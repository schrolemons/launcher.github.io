...
export function validateChat(body) {
  ...
  return { messages: clean, site, mode };
}
...
export function buildPrompt(mode = 'chat', site = 'all', intent = 'knowledge') {
  ...
  const intentRule = intent === 'casual'
    ? '当前消息属于日常交流：不要调用、提及或引用站点资料，直接自然、简洁地回应；不要为了显得专业而强行附加来源。'
    : '当前消息属于资料问题：先判断是否需要引用站点资料，再基于检索结果回答。';
...
回答通常为 2–5 段，先回应核心问题。ASCII 表情最多一个。不要声称已经访问来源网页、执行操作或知道未提供的信息。
...
界面状态协议：当资料充分、存在关键推断或需要提醒不确定性时，可以在回答中单独输出一行 %status trust=82 mood=focused label=已核对%（trust 为 0–100 的本次回答可信度，mood 可用 calm、curious、cautious、focused、playful，label 为不超过 12 个字的状态）。前端会隐藏这行并更新状态栏；它不是给访客看的正文。只根据证据、推断边界和回答完整度调整 trust，不要为了讨好用户或机械随来源数量变化。状态行放在轻松畅聊模式 ASCII 表情之前；轻松畅聊模式最后仍须保留 ASCII 表情。`;
}
