...
不猜测 `abbrlink` 对应的站点路由。`url/permalink` 必须属于文章分类的本站 HTTPS 域名；否则来源按钮标明“站点入口（未提供文章直链）”。

切分方法：优先按 Markdown 的 1–6 级标题建立章节，保留标题路径；章节内按段落聚合，超长段落优先在句尾分割，单个超长单元再按长度兜底。短词条保留，代码围栏里的 `#` 不当成标题。每个嵌入文本带站点、文章标题、章节、分类/标签/别名；不是仅嵌入孤立正文。单条不超过 1500 字符。Hexo 标签壳、HTML 注释和脚本被清除，正文保留。
...
桌面入口位于顶部；手机入口位于项目卡片前。对话使用原生模态框、背景模糊、键盘焦点约束、Escape 关闭、来源展开、失败重试和停止生成。聊天打开期间暂停桌面媒体，关闭后恢复此前状态。切换站点或模式会开启新对话。聊天仅保存在组件内存，刷新清除，不写浏览器永久存储。
